...
// state variable ; indicates that 68k isr is set
int irq;
...
int isr_poll() {
    ...
    // transfer in progress, no isr
    if (value & (1 << PIN_TXN_IN_PROGRESS))
        return irq;

    // ! PIN_IPL_ZERO => interrupt asserted
    if (!(value & (1 << PIN_IPL_ZERO)) ||
            // Software (emulated) interrupt asserted
            ipl_enabled[amiga_emulated_ipl()]) {
